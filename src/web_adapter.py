    ...
    def load_region(self, name: str) -> dict:
        if name not in self._region_cache:
            if name == "Brain":
                ...
                # A-P (patella rendered posterior, vertebral bodies on the wrong side).
                # The orientation letters are placed by convention, so that flip looked
                # labelled-right while the anatomy was reversed — verified by rendering.
