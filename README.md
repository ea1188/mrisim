...
- **Knee (real)** — a 3D isotropic T2 knee from the **KneeBones3Dify** dataset (Romano et al., [Zenodo](https://zenodo.org/records/10534328), CC-BY-4.0): bones from the provided masks (cortical + marrow), surrounding tissue classified from the real T2 intensity, and the real intensity as the texture field (`scripts/build_knee_atlas.py` → `data/knee_kb3d/`). TotalSegmentator MRI is torso-only, so the knee comes from this dedicated open dataset.
