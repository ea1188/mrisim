...
- **Sequences** — Spin Echo, FSE/TSE (EPG echo train), Gradient Echo, Inversion Recovery, Balanced SSFP (with off-resonance banding), Echo-Planar (EPI), Diffusion (DWI with ADC/FA maps), **Perfusion — arterial spin labeling (label-control + CBF maps) and dynamic contrast-bolus DSC/DCE (CBV / CBF / MTT / Ktrans maps)**, MR Angiography (TOF / Phase Contrast), Susceptibility-Weighted Imaging (SWI), fMRI BOLD, and Quantitative (qMRI) parameter mapping
...
- **Analysis & display** — a switchable signal curve (T2 decay / T1 recovery / TI sweep / flip-angle Ernst / contrast map / histogram) that can be hidden to give the image the full width, live k-space, pulse-sequence diagrams, tissue-label overlays, multi-slice grids, graphic FOV/slice planning, **3D reconstruction as a 2×2 quad (axial/coronal/sagittal reformats + a 3-D MIP overview from the acquired slab; also thick-slab MIP/MinIP/AIP, rotating MIP and oblique reformat)**, **on-image ruler & ROI measurement (distance, mean/SD/SNR — on the main image and each reconstruction panel)**, **receive-coil sensitivity shading** (surface vs array), **grayscale quantitative maps with calibrated colorbars** (radiology convention — the colorbar and unit label mark a map, not colour), clinical protocol presets, and SNR / CNR / SAR / scan-time metrics. The viewport carries DICOM-style corner annotations and anatomical orientation labels (radiological convention) on a dark "scanner-console" display. Scroll the wheel (or arrow keys) to page through contiguous slices a slice-thickness at a time, PACS-style. The control panel collapses into sections with a *find-a-control* search and editable numeric value fields.
...
- **Perfusion (DSC/DCE)** — dynamic contrast-bolus perfusion: a gamma-variate first-pass curve (DSC) → CBV / CBF / MTT (central-volume theorem) maps, and a Tofts uptake model (DCE) → Ktrans permeability — ~0 behind an intact blood-brain barrier, high in leaky tumour / active lesions (`dsc_dce.py`)
