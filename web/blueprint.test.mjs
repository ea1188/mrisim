...
test("PREMIUM_MAP: exactly the 21 audited premium topics, each mapped to a valid category", () => {
  const EXPECTED_TOPICS = [
    "instrumentation", "pulse-sequences", "data-acquisition", "contrast-weighting",
    "image-quality", "flow-artifacts", "fat-suppression", "three-d-recon", "perfusion", "mrs-advanced",
  ...
  assert.deepEqual(counts, { "image-production": 10, procedures: 8, safety: 1, "patient-care": 2 });
});
...
test("blend: free + premium sum per category with the enlarged denominator", () => {
  ...
  assert.equal(ip.memberCount, 14);                 // 4 free members + 10 premium topics
  assert.ok(Math.abs(ip.coverage - 4 / 14) < 1e-9);
