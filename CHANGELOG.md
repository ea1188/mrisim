...
### Changed
- **More web UI refinement.** Corners squared to a uniform 2px; the top-bar metric chips
  are now borderless stat readouts; the off-palette orange action buttons (Apply / Next /
  Restart) unified to the clinical accent blue; the top bar tightened and aligned with the
  protocol/quiz headers; the launcher now shares the app's font stack and background.
- Renamed the quiz page header to **"MRI Quizzes"**.

### Fixed
- Hardened a flaky protocol-planner smoke check (a re-render race after append/re-run).
...
### Added
- **Mobile-responsive Protocol Planning.** The planning workspace now adapts to phones:
  under 640px wide it stacks the protocol queue, scout viewports, and parameters into a
  single scrolling column with full-width viewports, and touch devices get larger tap
  targets. No separate mobile page is needed.
...
### Added
- **In-plane FOV box on image-library scouts.** Opening a sequence in an image-library exam
  now draws a movable/resizable in-plane FOV box on the scout whose plane matches the
  sequence — like a scanner's graphic prescription — so you can estimate the acquisition
  layout. The other two scouts keep their (illustrative) angle bands. Drag a corner to
  resize, drag inside to move; the box is remembered per sequence.
